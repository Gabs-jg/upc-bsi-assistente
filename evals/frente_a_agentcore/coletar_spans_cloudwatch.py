...
def ler_sessoes_capturadas(caminho):
    ...
    if not isinstance(captura, dict) or captura.get('estado') != 'concluido':
        raise ValueError('Use uma captura finalizada, sem erros.')
    resultados = captura.get('casos')
...
def main():
    ...
    p.add_argument('--horas', type=int, default=24) # Aumentado para 24
    ...
    if not 1 <= a.horas <= 48:
        raise SystemExit('Use intervalo de 1 a 48 horas.')
